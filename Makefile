.PHONY: help install build start stop restart logs status test dev dev-build dev-stop dev-restart dev-logs mock mock-all doctor backup update homeassistant-start homeassistant-stop music-start music-stop music-logs
...
help:
	...
	@echo "  make mock-all     Start development with mock Home Assistant and Music Assistant"
	...
	@echo "  make music-logs   Follow optional Music Assistant logs"
...
mock-all:
	HOME_ASSISTANT_MOCK=true MUSIC_ASSISTANT_MOCK=true $(DEV_COMPOSE) up --build
...
music-logs:
	docker compose --profile music logs --tail=100 --follow music-assistant-server
